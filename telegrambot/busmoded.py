...
# Пароли для ролей
ROLE_PASSWORDS = {
    'администратор': 'BayHarbor',  # Замените на ваш пароль администратора
    'водитель': 'Butcher',      # Замените на пароль для водителей
    'пассажир': 'DexterMorgan'    # Замените на пароль для пассажиров
}

# ID главного администратора
MAIN_ADMIN_ID = 1669252402  # Замените на ваш Telegram ID
...
async def selecting_priority(update, context):
    ...
    try:
        ...
        await query.edit_message_text(
            ...
            # Убираем reply_markup или устанавливаем reply_markup=None
        )
...
async def finish_route(update, context):
    ...
    if driver_id in routes:
        ...
        if route.is_open:
            ...
            optimized_pickup_locations, total_duration = optimize_route(
                origin=route.origin,
                destination=workplace_location,
